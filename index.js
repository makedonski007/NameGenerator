...
app.post("/submit", (req, res) => { // Handle POST request to /submit
  const newAdjective = adj[Math.floor(Math.random() * adj.length)]; // Select a random adjective
  const newNoun = noun[Math.floor(Math.random() * noun.length)]; // Select a random noun
  const bandName = `${newAdjective} ${newNoun}`; // Combine them to form a band name
  res.render("index.ejs", {bandName: bandName}); //1
});
